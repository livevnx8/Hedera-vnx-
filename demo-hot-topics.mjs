 ...
 *
 * Demonstrates the hot topics radar functionality
 */

import { hotTopicsManager, createHotTopicsScanner, INITIAL_MONITORED_TOPICS } from './dist/vera/orchestrator/index.js';
import { topicPoller } from './dist/vera/orchestrator/topicPoller.js';
  ...
  if (result.highVolume.length > 0) {
    ...
    for (const entry of result.highVolume) {
      ...
      if (entry.metadata?.classificationConfidence) {
        console.log(`     Confidence: ${(entry.metadata.classificationConfidence * 100).toFixed(1)}%`);
      }
    }
  } else {
