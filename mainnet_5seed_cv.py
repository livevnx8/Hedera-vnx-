#!/usr/bin/env python3
"""
5-seed cross-validation on mainnet data with stratified splits.
Reports mean ± std for statistical rigor.
"""
...
print("=" * 60)
print("5-SEED STRATIFIED CROSS-VALIDATION ON MAINNET DATA")
print("=" * 60)
...
print(f"After filtering (>=10 per class): {len(filtered)} samples, {len(valid_labels)} classes")
...
for seed in SEEDS:
    ...
    print(f"Test dist: {dict(Counter(item['label'] for item in test_raw))}")
    ...
    # Weights from REAL distribution
    ...
    print(f"Improvement: +{test_acc - majority_baseline:.3f}")
...
baselines = [r["majority_baseline"] for r in results]
improvements = [r["improvement"] for r in results]

print(f"Accuracy:     {np.mean(accs):.3f} ± {np.std(accs):.3f}  (range: {np.min(accs):.3f}-{np.max(accs):.3f})")
print(f"Baseline:     {np.mean(baselines):.3f} ± {np.std(baselines):.3f}")
print(f"Improvement:  {np.mean(improvements):.3f} ± {np.std(improvements):.3f}")

# Per-class across seeds
print(f"\nPer-class recall (mean ± std across 5 seeds):")
for c in sorted(valid_labels):
    recalls = [r["per_class_recall"][c] for r in results]
    name = type_map.get(c, f"class_{c}")
    print(f"  {name:25s}: {np.mean(recalls)*100:5.1f}% ± {np.std(recalls)*100:4.1f}%")

# Save
summary = {
    "dataset": "mainnet_real_5seed_cv",
    "total_samples": len(filtered),
    "seeds": SEEDS,
    "accuracy": {"mean": float(np.mean(accs)), "std": float(np.std(accs)), "min": float(np.min(accs)), "max": float(np.max(accs))},
    "baseline": {"mean": float(np.mean(baselines)), "std": float(np.std(baselines))},
    "improvement": {"mean": float(np.mean(improvements)), "std": float(np.std(improvements))},
    "per_class": {
        type_map.get(c, f"class_{c}"): {
            "mean": float(np.mean([r["per_class_recall"][c] for r in results])),
            "std": float(np.std([r["per_class_recall"][c] for r in results]))
        }
        for c in valid_labels
    },
    "runs": results,
...
out_path = Path("/home/vera-live-0-1/hedera-llm-api/mainnet_5seed_results.json")
